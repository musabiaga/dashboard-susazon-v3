...
type Medicion = "kg" | "venta" | "margenPct" | "margenAbs";
type Period = "aa_mes" | "aa_ytd" | "act_mes" | "act_ytd";
type SortCol = "name" | Period | "g_mes" | "g_ytd";

interface Row {
  name: string;
  aa_mes_venta: number; aa_mes_kg: number; aa_mes_margen: number;
  aa_ytd_venta: number; aa_ytd_kg: number; aa_ytd_margen: number;
  act_mes_venta: number; act_mes_kg: number; act_mes_margen: number;
  act_ytd_venta: number; act_ytd_kg: number; act_ytd_margen: number;
}
          ...
          <div className="flex min-w-full flex-col gap-0 lg:flex-row">
            ...
            <table className="w-full border-collapse text-xs lg:w-1/2">
              <thead className="sticky top-0 z-10" style={{ background: "var(--bg-surface-muted)" }}>
                <tr>
                  <th colSpan={3} className="px-2.5 pt-2 pb-0.5 text-left text-[11px] font-bold uppercase tracking-wider" style={{ color: "var(--text-secondary)" }}>
              ...
              <tbody>
                {sorted.map((r) => (
                  <tr key={"aa-" + r.name} style={{ borderBottom: "1px solid var(--border-subtle, var(--border))" }}>
                    <td className="max-w-[240px] truncate px-2.5 py-1.5" style={{ color: "var(--text-primary)" }} title={r.name}>{r.name}</td>
                    <td className="px-2.5 py-1.5 text-right tabular-nums" style={{ color: "var(--text-secondary)" }}>{fmtCell(cellValue(r, "aa_mes", medicion), medicion)}</td>
                    <td className="px-2.5 py-1.5 text-right tabular-nums" style={{ color: "var(--text-secondary)" }}>{fmtCell(cellValue(r, "aa_ytd", medicion), medicion)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            ...
            <table className="w-full border-collapse text-xs lg:w-1/2" style={{ borderLeft: "2px solid var(--border)" }}>
              <thead className="sticky top-0 z-10" style={{ background: "var(--bg-surface-muted)" }}>
                <tr>
                  <th colSpan={5} className="px-2.5 pt-2 pb-0.5 text-left text-[11px] font-bold uppercase tracking-wider" style={{ color: "var(--accent)" }}>
              ...
              <tbody>
                {sorted.map((r) => {
                  ...
                  return (
                    <tr key={"act-" + r.name} style={{ borderBottom: "1px solid var(--border-subtle, var(--border))" }}>
                      <td className="max-w-[200px] truncate px-2.5 py-1.5" style={{ color: "var(--text-primary)" }} title={r.name}>{r.name}</td>
                      <td className="px-2.5 py-1.5 text-right tabular-nums" style={{ color: "var(--text-primary)" }}>{fmtCell(cellValue(r, "act_mes", medicion), medicion)}</td>
                      <td className="px-2.5 py-1.5 text-right font-semibold tabular-nums" style={{ color: toneColor(gMes.tone) }}>{gMes.txt}</td>
                      <td className="px-2.5 py-1.5 text-right tabular-nums" style={{ color: "var(--text-primary)" }}>{fmtCell(cellValue(r, "act_ytd", medicion), medicion)}</td>
                      <td className="px-2.5 py-1.5 text-right font-semibold tabular-nums" style={{ color: toneColor(gYtd.tone) }}>{gYtd.txt}</td>
                    </tr>
                  );
                })}
              </tbody>
